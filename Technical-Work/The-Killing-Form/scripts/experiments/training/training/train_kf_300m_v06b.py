"""
Phase 4A-quinquies v0.6b: 300M HRM COUPLED Bidirectional KF Training

Based on train_kf_300m.py. KEY CHANGE: --kf_coupled applies bidirectional
gating to BOTH H and L modules (not H-only as in v0.6a).

Critical control for Finding #84: if v0.6b breathes but performs worse than
v0.6a, then separation + breathing is the combination.
...
def compute_l_module_cv_per_layer(raw_model, device):
    """Return list of (layer_idx, layer_cv_tensor, qkv_param_ref) for L-module gated KF."""
    l_module = raw_model.inner.L_level
    layer_data = []
    for layer_idx, layer in enumerate(l_module.layers):
        W = layer.self_attn.qkv_proj.weight
        num_heads = layer.self_attn.num_heads
        head_dim = layer.self_attn.head_dim
        W_Q = W[:num_heads * head_dim, :]
...
def main():
    ...
    parser.add_argument("--kf_objective", type=str, default="linear",
                        choices=["linear", "log", "adaptive", "gated", "bidirectional"],
    ...
    parser.add_argument("--kf_coupled", action="store_true",
                        help="Apply KF to BOTH H and L modules (v0.6b coupled control)")
    ...
    # Full-resolution breathing log (every KF application, not just every 10th)
    breathing_log_path = os.path.join(args.save_dir, "breathing_log.csv")
    breathing_log = open(breathing_log_path, "w")
    breathing_log.write("step,kf_reg,h_cv,h_build,h_neutral,h_dissolve,h_total,avg_cos_b,avg_cos_d,h_build_ids,h_dissolve_ids")
    if args.kf_objective == "bidirectional" and args.kf_coupled:
        breathing_log.write(",l_cv,l_build,l_neutral,l_dissolve,l_total,l_avg_cos_b,l_avg_cos_d")
    breathing_log.write("\n")
    breathing_log.flush()
        ...
        for set_name, batch, batch_size in chunk_loader:
            ...
            if args.kf_lambda > 0 and global_step % args.kf_every == 0:
                ...
                if args.kf_objective == "gated":
                    ...
                    # Full-resolution breathing log (EVERY application)
                    _avg_b = sum(c for _, c in h_build) / max(len(h_build), 1)
                    _avg_d = sum(c for _, c in h_dissolve) / max(len(h_dissolve), 1)
                    breathing_log.write("%d,%d,%.6e,%d,%d,%d,%d,%.6f,%.6f,%s,%s" % (
                        global_step, kf_reg_applications, h_cv_val.item(),
                        len(h_build), len(h_neutral), len(h_dissolve), h_total,
                        _avg_b, _avg_d,
                        ";".join(str(idx) for idx, _ in h_build) if h_build else "",
                        ";".join(str(idx) for idx, _ in h_dissolve) if h_dissolve else ""))
                    if args.kf_coupled:
                        _l_avg_b = sum(c for _, c in l_build) / max(len(l_build), 1)
                        _l_avg_d = sum(c for _, c in l_dissolve) / max(len(l_dissolve), 1)
                        breathing_log.write(",%.6e,%d,%d,%d,%d,%.6f,%.6f" % (
                            l_cv_val.item(),
                            len(l_build), len(l_neutral), len(l_dissolve), l_total,
                            _l_avg_b, _l_avg_d))
                    breathing_log.write("\n")
                    ...
                    # v0.6b: helper function for per-module bidirectional pass
                    def bidir_gated_pass(module_layers, cv_func, module_name, pre_saved_ce_grads):
                        """Run bidirectional gating on a set of layers.
                        pre_saved_ce_grads: dict of layer_idx -> grad tensor, saved BEFORE zero_grad."""
                        mod_ce_grads = pre_saved_ce_grads

                        layer_data = cv_func(raw_model, device)
                        cv_val = torch.stack([ld[1] for ld in layer_data]).mean()
                        ...
                        for layer_idx, layer_cv, param_ref in layer_data:
                            ...
                            if layer_idx in mod_ce_grads and param_ref.grad is not None:
                                kf_grad = param_ref.grad.detach().flatten()
                                ce_grad = mod_ce_grads[layer_idx].flatten()
                                cos_sim = torch.dot(kf_grad, ce_grad) / (
                                    torch.norm(kf_grad) * torch.norm(ce_grad) + 1e-20)
                                cos_val = cos_sim.item()
                                if cos_val > threshold:
                                    mod_build.append((layer_idx, cos_val))
                                elif cos_val < -threshold:
                                    param_ref.grad.mul_(-1.0)
                                    mod_dissolve.append((layer_idx, cos_val))
                                else:
                                    param_ref.grad.zero_()
                                    mod_neutral.append(layer_idx)

                        return cv_val, mod_build, mod_neutral, mod_dissolve, len(layer_data)

                    # BUG FIX: Save CE grads BEFORE zero_grad wipes them!
                    h_ce_grads = {}
                    for layer_idx, layer in enumerate(raw_model.inner.H_level.layers):
                        p = layer.self_attn.qkv_proj.weight
                        if p.grad is not None:
                            h_ce_grads[layer_idx] = p.grad.detach().clone()

                    l_ce_grads = {}
                    if args.kf_coupled:
                        for layer_idx, layer in enumerate(raw_model.inner.L_level.layers):
                            p = layer.self_attn.qkv_proj.weight
                            if p.grad is not None:
                                l_ce_grads[layer_idx] = p.grad.detach().clone()

                    optimizer.zero_grad()

                    # Always process H module
                    h_cv_val, h_build, h_neutral, h_dissolve, h_total = bidir_gated_pass(
                        raw_model.inner.H_level.layers,
                        compute_h_module_cv_per_layer, "H", h_ce_grads)

                    if args.kf_coupled:
                        # v0.6b COUPLED: also process L module with bidirectional gating
                        l_cv_val, l_build, l_neutral, l_dissolve, l_total = bidir_gated_pass(
                            raw_model.inner.L_level.layers,
                            compute_l_module_cv_per_layer, "L", l_ce_grads)
                    else:
                        # v0.6a DEFAULT: zero L-module grads (separation of concerns)
                        for p in raw_model.inner.L_level.parameters():
                            if p.grad is not None:
                                p.grad.zero_()
                    ...
                    if kf_reg_applications % 10 == 0:
                        avg_b = sum(c for _, c in h_build) / max(len(h_build), 1)
                        avg_d = sum(c for _, c in h_dissolve) / max(len(h_dissolve), 1)
                        print("  [Step %d] KF-bidir #%d: H_CV=%.6e  build=%d  neutral=%d  dissolve=%d  /%d  avg_cos_b=%.4f  avg_cos_d=%.4f" % (
                            global_step, kf_reg_applications, h_cv_val.item(),
                            len(h_build), len(h_neutral), len(h_dissolve), h_total,
                            avg_b, avg_d), flush=True)
                        if args.kf_coupled:
                            l_avg_b = sum(c for _, c in l_build) / max(len(l_build), 1)
                            l_avg_d = sum(c for _, c in l_dissolve) / max(len(l_dissolve), 1)
                            print("           L-module: L_CV=%.6e  build=%d  neutral=%d  dissolve=%d  /%d  avg_cos_b=%.4f  avg_cos_d=%.4f" % (
                                l_cv_val.item(),
                                len(l_build), len(l_neutral), len(l_dissolve), l_total,
                                l_avg_b, l_avg_d), flush=True)
                else:
    ...
    print("\n" + "=" * 70)
    breathing_log.close()
    print("Breathing log saved to: %s" % breathing_log_path, flush=True)
