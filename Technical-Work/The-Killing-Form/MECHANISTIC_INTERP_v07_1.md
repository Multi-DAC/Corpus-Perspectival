...
**Mechanism tally — ALL FOUR tested at multi-seed (n=5):** #2 OV-direction (ruled out), #4 gating-frame (ruled out), #3 effective-rank (not supported, gap ≈ 0), #1 functional-specialization (not supported, marginally *reversed*: gap −0.0036 ± 0.0035, 4/5 seeds negative, paired t=−2.07 — v0.7.1's classes write slightly *less* functionally-distinct than baseline's). **All four proposed pathways for the orthogonality effect systematically eliminated.** Subtle finding: norm-class-separation (what the aux imposes) does NOT correspond to functional-write-separation; if anything the opposite.
