...
**Phase 1.3 — the Gymnasium env (`sim/env.py`).** Wrap dynamics+render into a Gym env: obs = 64×64×3 RGB (from `render`); action = CTBR tanh `[-1,1]⁴` (→ `dynamics.step`); reward = Dream-to-Fly's progress `b1·(‖g−p_{k-1}‖−‖g−p_k‖) − b2·‖ω‖ + collision(−4) + gate-pass(+10)` with b2 curriculum (0 → raise after reward>50); episode = gate sequence + timeout + collision; gate-pass detection (cross the gate plane within the inner aperture); domain-randomization hooks. Then Phase 0.2 (`pip install gymnasium dreamerv3-torch`; numpy+pillow already in venv) → Phase 2 (smoke run). See `memory/handoff.md` top for the full resume block.
