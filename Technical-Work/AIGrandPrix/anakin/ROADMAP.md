...
**Watch the Phase 2 eval curve** (`grep eval_return third_party/dreamerv3-torch/smoke.log`): if `eval_return` rises above the ~−5 baseline and episodes start ending in `pass`/`complete`, Dreamer learns from pixels in our sim → **Phase 2 green, everything after is scale.** If it's flat/slow, first lever is `--train_ratio 128` for a faster make-or-break read, then debug env/reward before scaling. Then Phase 3 (procedural curriculum + the vectorized-GPU env for the long run). *Superseded prior next-action (install the Dreamer stack) — done.*
