...
chosen from a continuum of options. Three complementary perspectives
on a single geometric fact---that the scalar is a metric fluctuation---
converge on this value: the Seeley--DeWitt $(a_2)$ heat-kernel
coefficient, Weyl conformal-invariance of the scalar kinetic term, and
the radion identification as the conformal fluctuation of the 5D metric.
Each illuminates a different facet of the same geometry and yields the
same number; the full derivations appear in
Appendix~\ref{app:xi}.
...
\textit{Three complementary perspectives fix $\xi = 1/6$.} The
Seeley--DeWitt $(a_2)$ coefficient, Weyl conformal-invariance of the
scalar kinetic term, and the radion identification as the conformal
fluctuation of the 5D metric all yield $\xi = 1/6$. These are not
seven independent proofs: they are three facets of a single geometric
fact---that the scalar is a metric fluctuation---each operating on
different degrees of freedom (heat-kernel expansion, Weyl scaling,
Kaluza--Klein reduction) but measuring the same object. See
Appendix~\ref{app:xi}.

\textit{Algebraic maximum determines $N_g = 3$.} The number of fermion
generations is not an input. The exceptional Jordan algebra
$J_3(\mathbb{O})$ pins $N_g \leq 3$ as an algebraic maximum, and three
independent perspectives (Dixon algebra grading
$\mathbb{C} \otimes \mathbb{H} \otimes \mathbb{O}$, anomaly cancellation
on the orbifold, and Clifford periodicity) confirm the maximum is
saturated. The algebra sets the ceiling; the sector checks show it is
reached. See Appendix~\ref{app:predictions} S6 and
Chapter~\ref{ch:ncg}.
...
canonical home is Appendix~\ref{app:val-tier2}.
...
observation. The complete canonical registry comprises fifteen
falsifiable predictions---seven structural (S1--S7) and eight parametric
(P1--P8)---tabulated with computational provenance in
Appendix~\ref{app:predictions}. The summary below is a chapter-level
view highlighting the most discriminating entries; Appendix~B is
authoritative.
...
falsifiable predictions (a chapter-level development; see
Appendix~\ref{app:predictions} for the canonical fifteen-entry registry).
...
% ============================================================
\section{Notation}
\label{sec:0-notation}

Several closely-named symbols recur across the chapters. The following
table collects them with a one-line gloss and the section where each
is defined in full.
...
\begin{tabular}{@{}llll@{}}
\hline
Symbol & Role & Canonical value & Defined \\
\hline
$\xi$              & Non-minimal scalar--curvature coupling    & $1/6$ (conformal)     & App.~A \S\ref{app:xi} \\
$\CGB$             & Gauss--Bonnet junction factor              & $2/3$                 & Ch.~\ref{ch:ncg}, App.~A \\
$\eps$ (=$\epsilon_1$) & First NCG correction to $\CGB$          & $\approx 0.010$        & Ch.~\ref{ch:ncg} \\
$\epsilon_2$       & Higher Seeley--DeWitt correction           & $<5\%$ at benchmark   & Ch.~\ref{ch:sound-speed} \\
$\varepsilon$      & Goldberger--Wise scaling dimension         & $0.275$ (fitted)      & Ch.~\ref{ch:foundation} \\
$\hat{\alpha}$     & Cutoff-sensitivity coefficient             & App.~A Tier 2          & App.~A \\
$\zz$              & Cuscuton mass-scale parameter              & $0.016 \pm 0.002$     & Ch.~\ref{ch:foundation}, App.~D \\
$\wz$              & Cuscuton dark-energy equation of state     & $-0.990$ (weighted)    & Ch.~\ref{ch:foundation}, App.~D \\
$\CKK$             & KK-mode spectral contribution              & $(1.64 \pm 0.33)\times 10^{-4}$ & App.~D Tier 2 \\
\hline
\end{tabular}
\end{center}

The two epsilons are notationally close but operationally distinct.
$\varepsilon$ is the externally-fitted Goldberger--Wise scaling
dimension controlling the radion--cuscuton coupling; $\eps$ (with a
slanted glyph) is the first-order NCG correction to the Gauss--Bonnet
junction factor $\CGB$. They belong to different sectors of the
derivation chain and cannot be confused in context, but readers
skimming equations out of order may find the distinction helpful.
...
% ============================================================
\section{The Philosophical Frame: Meridian as a Partial Map}
\label{sec:0-anchor}

This volume is a \emph{partial map of our specific physical stream}
within the physical aspect of reality---where ``physical aspect'' is
one of several aspects of the larger structure the Anchor volume
\emph{The Coherence Principle} names $X$, and each aspect
accommodates multiple streams. Meridian does not attempt to derive
the physical aspect as such; it derives the structure of this basin
(5D warped geometry, NCG spectral triple, cuscuton kinetic function,
Goldberger--Wise stabilisation) to the depth the current apparatus
allows, with explicit scope and explicit remainders
(Appendix~\ref{app:val-apophatic}).

Three structural features of the monograph are instances of the
Coherence Principle applied to this stream:
\begin{itemize}\setlength{\itemsep}{2pt}
  \item \textbf{Growth--expansion decoupling.} The cuscuton's
  zero-DOF kinetic structure gives $\mu = \Sigma = 1$
  (Chapter~\ref{ch:observational}, Chapter~\ref{ch:sound-speed})---a
  measurement-channel separation that the Principle predicts
  wherever a coherent substrate operates on distinct degrees of
  freedom.
  \item \textbf{Cuscuton zero-DOF coherence.} The infinite-symmetry
  protection of $P(X) = \mu^2\sqrt{2X}$
  (Chapter~\ref{ch:foundation}, Chapter~\ref{ch:nogo}) is the
  Principle's dynamic-maintenance condition: the basin maintains
  self-tuning not by tuning initial conditions but by having
  strictly fewer propagating modes than apparent.
  \item \textbf{One-parameter-with-external-$\varepsilon$ boundary.}
  The spectral chain $\mathbb{O} \to \mathrm{M}_\mathrm{oct} \to$
  Yukawa $\to b_{3/2} \to \alpha_\mathrm{UV} \to \mu^2(\varepsilon)
  \to \zeta_0 \to w_0$ (Appendix~\ref{app:gw-implications})
  terminates at a single external input---$\varepsilon$ fitted to
  DESI---rather than at zero parameters. The Principle predicts that
  any stream-local map will have such an external input: the
  measurement dimension through which the basin couples to the
  wider structure.
\end{itemize}

This framing does not modify the physics. Every derivation stands on
its own merits, as presented in the chapters that follow. What the
framing adds is scope honesty: the claim is not ``Meridian is the
theory of dark energy'' but ``Meridian is the partial map of how
dark energy arises in \emph{this} basin, with the map extending to
the depth the NCG + GW apparatus presently reaches, and with the
remainder named in two classes
(Appendix~\ref{app:val-apophatic}).''

For the philosophical framework, see
\emph{The Coherence Principle} (Library volume, Anchor;
Zenodo DOI 10.5281/zenodo.19634474). The Anchor volume develops the
stream/basin vocabulary, the four coherence conditions, and the
inspection-depth discipline this monograph draws on.
