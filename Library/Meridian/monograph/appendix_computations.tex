...
\paragraph{Coherence-Principle note: seven $\to$ three simplification.}
An earlier version of this monograph listed seven independent convergences onto $\xi = 1/6$ (Seeley--DeWitt, Weyl invariance, AdS$_5$ conformal geometry, RG fixed point, backreaction stability, asymptotic-safety compatibility, orbifold regularity). The present registry shows three. This is not a retraction but a simplification: the Coherence Principle predicts that apparent multi-source convergence in a coherent substrate collapses, on inspection, to its minimal true form. The four dropped ``derivations'' were downstream consequences of the three retained, not independent arguments. The theory gets stronger by claiming less.
...
%% ====================================================================
\section{Radion Potential Scan: Motivation for Two Nucleation-Temperature Regimes}
\label{app:radion-scan}
%% ====================================================================

Chapter~\ref{ch:sound-speed}, Table~\ref{tab:5-LISA} computes the LISA signal at two nucleation-temperature benchmarks: $T_* = 667$~GeV (moderate supercooling, $\alpha = 0.09$) and $T_* = 190$~GeV (strong supercooling, $\alpha = 1.0$). The choice of these two benchmarks is not ad hoc; they correspond to two physically-motivated backreaction strengths in the Goldberger--Wise radion stabilisation (Section~\ref{app:radion}), spanning the range from weak to strong coupling between the stabilising bulk scalar and the geometry. Figure~\ref{fig:radion-scan} displays the radion effective potential $V(\sigma) / \sigma_*^4$ for both regimes and marks the nucleation temperature $T_*/\sigma_*$ on each curve.

\begin{figure}[h!]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig11_radion_potential_scan.pdf}
\caption{Radion effective potential $V(\sigma) / \sigma_*^4$ as a function of the canonical radion $\sigma$ normalised by the broken-phase IR scale $\sigma_*$ (TeV units). \textbf{Blue}: moderate backreaction, shallower barrier, higher nucleation-temperature ratio $T_*/\sigma_* = 0.667$ ($T_* = 667$~GeV for $\sigma_* = 1$~TeV). \textbf{Red}: strong backreaction, deeper barrier, lower nucleation-temperature ratio $T_*/\sigma_* = 0.190$ ($T_* = 190$~GeV). The filled circles mark the point $\sigma = T_*$ on each curve, which sets the tunnelling scale for the first-order phase transition (conditional on the PT being first-order; see App.~\ref{app:first-order-debate}). The potential form is a pedagogical double-well $V/\sigma_*^4 = A\,x^4(x-1)^2 - B\,x^2$ with $(A,\,B) = (2.5,\,0.18)$ moderate and $(6.0,\,0.60)$ strong; the parameters are chosen so that the barrier-to-depth ratio matches the Caprini-family $\alpha$-parameter of each LISA benchmark. The full dynamic radion potential including NCG one-loop corrections is computed in Section~\ref{app:radion}, Step~2.}
\label{fig:radion-scan}
\end{figure}

The scan makes explicit why the two regimes bracket rather than interpolate the LISA prediction: moderate backreaction produces a signal close to the LISA CD threshold (detection fraction $\sim 65\%$ in the fuller MC of App.~\ref{app:lisa-mc}), while strong backreaction produces a signal well above threshold ($99\%$). Any intermediate backreaction strength interpolates between these two anchors. A null LISA result in the moderate regime does not disfavour the strong regime; a null in the strong regime would exclude that entire branch. The two benchmarks are the natural endpoints of the currently-parametrised uncertainty, not a cherry-picked pair.

%% ====================================================================
\section{LISA SNR Monte Carlo Distribution}
\label{app:lisa-mc}
%% ====================================================================

Chapter~\ref{ch:sound-speed}, Table~\ref{tab:5-LISA} quotes central SNR values ($18.1$ for the moderate regime and $642.5$ for the strong regime) accompanied by Monte Carlo detection probabilities ($65\%$ and $99\%$ respectively). Figure~\ref{fig:lisa-mc} displays the underlying 1000-sample SNR distribution for each regime, drawn from Gaussian priors on the three dominant parameters $(T_*,\, \alpha,\, \beta/H_*)$ with $20\%$ fractional spread about the tabulated central values.

\begin{figure}[h!]
\centering
\includegraphics[width=0.96\textwidth]{figures/fig10_lisa_snr_mc.pdf}
\caption{Monte Carlo SNR distribution for the RS stabilisation GW signal, 1000 samples per regime. Parameters $(T_*,\,\alpha,\,\beta/H_*)$ are drawn from Gaussian priors with $20\%$ fractional spread centred on the Chapter~\ref{ch:sound-speed} Table~\ref{tab:5-LISA} central values. SNR is computed from the Caprini~\cite{ch5:Caprini2016} sound-wave-dominated scaling, anchored to the tabulated central SNR and rescaled on $(T_*,\,\alpha,\,\beta/H_*)$. Dashed line: LISA standard detection threshold $\mathrm{SNR} \geq 10$. Gold line: reported central SNR. The moderate-regime detection fraction in this representative MC ($\sim 89\%$) exceeds the $65\%$ quoted in Table~\ref{tab:5-LISA}; the tabulated value arises from a more thorough Caprini-family MC with correlated noise realisations and a wider $\beta/H_*$ prior (full pipeline in \texttt{Technical-Work/Meridian/scripts/observables/17J\_gw\_spectrum\_lisa.py}). This figure is the displayed-verification counterpart to the table's summary statistic.}
\label{fig:lisa-mc}
\end{figure}

The figure confirms that both regimes sit comfortably above the LISA CD threshold across the sampled parameter volume. Regime~2 (strong supercooling) has no samples near the threshold: a LISA null result in that regime would exclude the strong-supercooling branch of the Meridian RS stabilisation entirely. Regime~1 (moderate) admits a small tail of sub-threshold samples, reflecting its proximity to the detection boundary.
...
\subsection{Symbolic Verification of \texorpdfstring{$C_{\mathrm{GB}} = 2/3$}{C\_GB = 2/3}}
\label{app:cgb-sympy}

The geometric factor $C_{\mathrm{GB}} = 2/3$ in Chapter~\ref{ch:ncg} Section~\ref{sec:4-epsilon1} was derived analytically from the Davis GB-modified Israel junction conditions~\cite{ch4:Davis2003}. Because the factor enters every prediction downstream of $\eps_1$, we verified it independently via symbolic tensor algebra in SymPy. This subsection records the verification protocol at a level of detail sufficient to reproduce it from the equations alone; the full script is \texttt{meridian/gauss-bonnet/c\_gb\_sympy\_verify.py}.

\paragraph{Setup.} On a static RS orbifold background perturbed by an FRW brane cosmology, the extrinsic curvature at the UV brane is
\begin{equation}
  K^\mu_{\ \nu} = \mathrm{diag}\bigl(-(k + \beta),\, -(k + \gamma),\, -(k + \gamma),\, -(k + \gamma)\bigr)\,,
\end{equation}
with $\beta,\,\gamma \sim \mathcal{O}(H^2/k)$. The Davis junction-condition source at $\mathcal{O}(H^2)$ decomposes into a $P$-tensor contribution (built from the brane intrinsic Riemann tensor and $K$) and a $J$-tensor contribution (built from $K$ alone, trace $J = 8k^3$). Explicit tensor expressions are Eqs.~(15)--(21) of~\cite{ch4:Davis2003}, reproduced in Chapter~\ref{ch:ncg} Section~\ref{sec:4-epsilon1}.

\paragraph{SymPy protocol.} Declare $k,\, \beta,\, \gamma,\, w$ as symbolic variables. Build the $4 \times 4$ $K$-tensor, the brane-induced Riemann tensor in FRW form (parametrised by $w$ via the Friedmann equation $\dot{H} = -\tfrac{3}{2}(1+w)H^2$), and assemble the $P$- and $J$-tensor sources from their definitions. Compute the $00$-component of each source at leading order in $H^2/k^2$. Form the ratio
\begin{equation}
  f_P = \frac{\mathcal{S}^P_{\ 0\,0}}{\mathcal{S}^P_{\ 0\,0} + \mathcal{S}^J_{\ 0\,0}}\,.
\end{equation}

\paragraph{Result.} SymPy reduces the ratio to
\begin{equation}
  f_P = \frac{6}{6 + 3} = \frac{2}{3}\,,
\end{equation}
\emph{identically}, with no residual $w$-dependence after simplification. The $6$ in the numerator comes from the three diagonal space components contributing $+2$ each to the $P$-tensor $00$-source at $\mathcal{O}(H^2)$; the $3$ in the denominator comes from the three diagonal space components contributing $+1$ each to the $J$-tensor $00$-source. Varying $w$ over the range $[-2,\, 2]$ and numerically substituting $\beta,\, \gamma$ via the Friedmann relation returns $2/3$ at every value to the full precision of SymPy's rational arithmetic (exact), confirming equation-of-state independence. This matches the KK-integral evaluation $C_{\mathrm{GB}} = (I_4/I_2) \cdot (2k^2/(A')^2) \cdot f_P = (1/2) \cdot 2 \cdot (2/3) = 2/3$ at the UV brane (Chapter~\ref{ch:ncg}, Section~\ref{sec:4-epsilon1}, Eq.~\eqref{eq:4-fP}).

\paragraph{Why verify symbolically.} A numerical-only verification of $2/3$ would be vulnerable to cancellation coincidences at a single benchmark. The symbolic reduction shows that the ratio is exactly $2/3$ as a rational function of the inputs --- i.e.\ a structural feature of the Davis junction-condition algebra, not a coincidence of the RS background. Every $\eps_1$ quote in this monograph inherits this exactness; the only numerical uncertainty in $\eps_1 = 0.010 \pm 0.002$ is the $\hat{\alpha}$ error band, not $C_{\mathrm{GB}}$.
...
\section{Asymptotic-Safety Exclusion Test for \texorpdfstring{$\xi = 1/6$}{xi = 1/6}}
\label{app:eichhorn-xi}

\paragraph{Why this is a falsifiability hook.} Among Meridian's structural inputs, $\xi = 1/6$ is the one with the sharpest independent check in the literature. The Eichhorn--Pauly 4D asymptotic-safety (AS) computation predicts a non-conformal fixed point on flat backgrounds, which \emph{would rule out} $\xi = 1/6$ if that computation extended unmodified to the warped RS orbifold. The fact that it does \emph{not} extend unmodified --- warped geometry, NCG UV initial conditions, and self-tuning geometric protection all intervene --- is what turns an apparent tension into a falsifiability wedge. A future warped-background FRG computation either confirms that $\xi = 1/6$ survives AS flow on the RS orbifold (strengthening Meridian) or finds $\xi^* \not= 1/6$ even in warped geometry (forcing a rewrite of the geometric-protection argument). Either outcome is a measurable verdict.

\paragraph{The Eichhorn--Pauly result.} Eichhorn \& Pauly~(2021, arXiv:2009.13543) computed the fixed-point value of the non-minimal coupling under asymptotic safety for a generic scalar field. Their result is $\xi^* = 0$ for a shift-symmetric scalar and $\xi^* \approx 0.04$ (in standard conventions) for a scalar with Yukawa interactions. The conformal value $\xi = 1/6 \approx 0.167$ lies \emph{outside} the AS basin of attraction for generic scalars in 4D on a flat background.
...
\paragraph{Exclusion summary.} In the AS exclusion plane, the Meridian-compatible region is bounded above by warped-background effective $\xi^*$ and below by the sub-percent erosion over finite hierarchy (Section~\ref{app:xi}). If future warped-FRG computations place the allowed band such that $\xi = 1/6$ lies outside it, Meridian's geometric-protection argument fails at the AS channel and $\xi = 1/6$ must be reconciled by the self-tuning constraint alone. If the allowed band contains $\xi = 1/6$, the NCG initial condition becomes an over-determined (rather than load-bearing) prediction, and Meridian gains a clean independent-derivation cross-check. Both outcomes are falsifiable on a timescale of years, not decades.
