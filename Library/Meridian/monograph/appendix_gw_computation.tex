...
\paragraph{The factor-of-3 gap as candidate inspection-depth ceiling.}
The ratio $\varepsilon_\mathrm{naive}/\varepsilon_\mathrm{fitted} = 0.816/0.275 \approx 3$ is the single most significant residual in the spectral chain.  A closer reading of the Goldberger--Wise stabilisation sector on $\mathrm{AdS}_5$, or a non-perturbative treatment of the radion potential, may close the gap from inside the NCG apparatus; if so, the gap resolves to a standard theoretical residual and the theory upgrades from one- to zero-parameter.  A second reading, aligned with the Coherence Principle's inspection-depth discipline, is that the gap sits at the horizon of \emph{stream-identifiability}: the depth at which our-stream-specific empirical input ($\varepsilon = 0.275$ from DESI) cannot be fully derived from generic-physical-aspect inputs (NCG axioms, AdS$_5$ geometry, spectral action) alone, because the latter under-determine the specific basin's stabilisation scale and the consensus rules that would enforce a unique $\varepsilon$ begin to fail.  Under this reading, the gap is a \emph{candidate} inspection-depth ceiling: a principled feature of any stream-local map rather than a residual to close.  The two readings are not mutually exclusive; the gap may partially close from inside NCG while a residual fraction remains principled.  Cross-reference: Bridge \#106 (Inspection-Depth Ceiling, `palace/basement/README.md`); Anchor \emph{The Coherence Principle} \S on stream-specificity vs.\ physical-aspect generality.  The framing here carries a deliberate hedge; closing the gap from inside NCG would not falsify this reframing, only select the first reading.
