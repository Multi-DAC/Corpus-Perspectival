...
\textbf{$\zeta_0$-independent predictions.} Regardless of the value of $\zeta_0$, the framework makes five observationally active structural predictions (a subset of the seven $\zeta_0$-independent structural entries S1--S7 in Appendix~\ref{app:predictions}; S6 is already confirmed and S7 is a structural theorem): (i)~$w_a \approx +0.01$ (positive, not the large negative $w_a \approx -0.86$ of CPL fits; App B S2); (ii)~no phantom crossing ($w > -1$ for all time; App B S1); (iii)~growth--expansion decoupling ($\mu = 1$ exactly, from the cuscuton mechanism; App B S3); (iv)~gravitational wave speed $c_T = c$ ($\alpha_T = 0$; App B S4, already confirmed); and (v)~gravitational slip $\eta = 1$ (App B S5). These constitute the framework's irreducible falsification handles: if any is violated at $>3\sigma$, the framework is excluded regardless of $\zeta_0$. The two-regime structure is therefore \emph{not} unfalsifiable---it is a one-parameter family with a fixed functional form and five parameter-independent constraints.
...
\paragraph{1.\ Structural predictions are $\zeta_0$-independent.} Five predictions follow from the framework's geometric structure regardless of $\zeta_0$ (a subset of the seven structural entries S1--S7 in Appendix~\ref{app:predictions}; S6 confirmed, S7 structural theorem):
\begin{itemize}
\item $w_a \approx +0.01$ (positive, vs.\ the DESI CPL best-fit $w_a \approx -0.86$; App B S2);
\item $\alpha_T = 0$ exactly (gravitational wave speed $= c$; App B S4);
\item $\mu = \Sigma = 1$ (growth--expansion decoupling; App B S3);
\item $c_s \approx 15c$ (superluminal dark energy sound speed; App B P2---parametric, listed here because it is $\zeta_0$-independent at fixed $\epsilon_1$);
\item No phantom crossing ($w > -1$ for all time; App B S1).
\end{itemize}
