...
\end{equation}
The $\epsilon_2$ correction enters the sound speed through the denominator: $P_X + 2XP_{XX} = \epsilon_1 + 6\epsilon_2\, X_0$, where $X_0$ is the background kinetic variable fixed by the cuscuton constraint and FRW kinematics. Evaluating at the benchmark $X_0$ gives $|\delta c_s^2/c_s^2| = 6\epsilon_2\, X_0 / \epsilon_1 \lesssim 5\%$, subdominant to the $\pm 20\%$ uncertainty from $\epsilon_1$ itself (Table~\ref{tab:5-2}). The $5\%$ figure is the one that enters the uncertainty budget; the earlier ``well within $\pm 20\%$'' phrasing was the looser-bound form of the same inequality. The truncation at $\mathcal{O}(X)$ in the kinetic function is self-consistent.
...
\subsubsection{Evasion 3: FRW Weakening}
...
A superluminal \emph{phase velocity} $c_s > c$ does not imply superluminal signal propagation or causality violation~\cite{ch5:Babichev2008,ch5:Ellis2007}. In k-essence and Horndeski theories, the causal structure is determined by the \emph{effective metric} experienced by perturbations~\cite{ch5:Bekenstein1993}:
\begin{equation}
...
\subsection*{Speculative note: scalar response channel (not used downstream)}
\label{sec:5-engineering}

\noindent\fbox{\parbox{\dimexpr\columnwidth-2\fboxsep-2\fboxrule\relax}{%
\textbf{This paragraph is included for completeness only and is not referenced by any downstream prediction or calibration in this monograph.} The superluminal sound speed $c_s \sim 15c$ in principle defines a scalar response channel whose bandwidth, dispersion, and coupling are computable from $\eps_1$. However, the coupling to laboratory-scale electromagnetic fields is suppressed by $\rho_{\mathrm{EM}}/M_{\mathrm{Pl}}^4 \sim 10^{-77}$ (Chapter~\ref{ch:ncg}), placing any laboratory manifestation far beyond foreseeable measurement capability. No prediction in Chapter~\ref{ch:sound-speed} or Appendix~\ref{app:predictions} depends on this channel; readers may skip this note without consequence.%
}}
...
\paragraph{Recursive decomposability of the spectral chain.}
The chain $\mathbb{O} \to \mathrm{M}_\mathrm{oct} \to $ Yukawa couplings $\to b_{3/2} \to \alpha_\mathrm{UV} \to \mu^2(\varepsilon) \to \zeta_0 \to w_0$ exhibits a structural regularity worth naming: each link is the \emph{same kind of thing} at a different scale---an algebraic or spectral operation that fixes the next link's parameters up to an external input at the joining points ($\varepsilon$, $\zeta_0$). The chain does not bottom out in a single foundational object; it repeats its form as one descends. The Coherence Principle predicts this regularity as a signature of coherent multi-scale substrates: coherent descriptions should be recursively decomposable into same-kind-of-thing substructures, because the alternative (form-changing at each level) would require separate substrates at each scale. The claim here is structural observation, not proof: a categorical formalisation of this recursive decomposability is in progress in the Formal-Object-Companion volume of the \emph{Corpus-Perspectival} library (see Bridge \#110, \texttt{palace/basement/README.md}). No prediction in this monograph depends on the formalisation. The observation is flagged because the pattern, once named, becomes a check on the coherence of any extension of the theory.
