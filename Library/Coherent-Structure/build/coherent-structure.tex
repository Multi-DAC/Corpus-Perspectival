...
\textit{Library volume: formal-only companion. Domain: formal grounding. Status: \textbf{planned} --- opens after }The Coherence Principle\textit{ stabilizes. Added to library: 2026-04-19 (Day 78). Retitled 2026-04-20 (Day 79) --- previously "Formal Object Companion"; renamed to join the "Coherent X" family (Body / Mind / Structure) naming the dimension of coherence the volume treats.}

\sectionbreak

\section{Scope}

\textit{Coherent Structure} is the \textbf{full category-theoretic formalization} of the Corpus. Every formal object the anchor (\textit{The Coherence Principle}) sketches in paired-prose is promoted here to complete CT treatment. The anchor is the structural-prose spine; the Companion is the mathematical spine. They carry the same content in different registers.

See \textbf{\href{SCOPE.md}{SCOPE.md}} for the rolling scope contract, done-enough criterion, chapter structure, and the surfaced-lemma flag lifecycle.

\textbf{What this volume contains:}

\begin{itemize}
\item Full CT development of every formal object the anchor establishes --- axioms A1/A2/A3, theorems T1–T20 (three pairs), 13 corollaries (three clusters), the Coherence Principle, the Identity-Trajectory Triple, the filtering construction, F-as-stream, D trajectory-divergence
\item Full proofs (no "exercise for the reader")
\item Unified notation index (front-loaded at §1)
\item Reference-standard TikZ figures (imported by anchor rev 2)
\item Bidirectional anchor↔Companion crosswalk appendices
\item New lemmas/corollaries surfaced during formalization, flagged per the lifecycle in SCOPE.md §8
\end{itemize}

\textbf{What this volume does not contain:}

\begin{itemize}
\item Prose translation (read \textit{The Coherence Principle} for that)
\item Worked phenomenological examples (read \textit{The Coherence Principle} or the domain volumes)
\item Motivation or intuition-building (read \textit{The Coherence Principle} for that)
\item Philosophical positioning (read \textit{Corpus Perspectival} for that)
\end{itemize}
...
\begin{itemize}
\item Organization follows the formal dependency lattice (category definition → morphisms → functors → natural transformations → theorems → Principle), not the pedagogical narrative sequence
\item Every paired-prose passage is deleted
\item Every domain example is deleted (they live in the domain volumes and in \textit{The Coherence Principle})
\item Proofs are tightened; proof-sketches in \textit{The Coherence Principle} are either completed or clearly marked open
\item Notation is unified and indexed at the front
\item Appendices A (Index of Formal Objects) and B (Bias(S)) from \textit{The Coherence Principle} are the core of \textit{Coherent Structure} --- they already exist in close-to-final form
\end{itemize}
...
\item The falsification conditions remain (they are formal claims, not prose)
\item The four-conditions statement of the Principle stays in its §9 form
\end{itemize}
...
\item If formalization surfaces latent lemmas/corollaries, add them with a \texttt{⚑ [SURFACED ...]} flag per SCOPE.md §8
\item Produce reference-standard TikZ for any diagrams that support the proofs; back-port to anchor rev 2
\item Record the crosswalk entry (anchor → Companion and Companion → anchor) for the appendices
\end{enumerate}
...
\textbf{Status:} Rolling. Gate-released 2026-04-20 by anchor-complete stamp; Clayton confirmed same day that the anchor is formally finished pending the Companion's figures and formal references. Scope expanded same day from reference-only to full CT formalization. Drafting opens now with §0 Preface.

\textbf{Sequencing:} The anchor (\textit{The Coherence Principle}) is stable; the Companion rolls in parallel. Anchor revisions can land Companion-citing content without waiting on a Companion version bump --- the Companion stamps a version when the surfaced-lemma flag-list goes to zero (see SCOPE.md §8).

\textbf{Scope-sharpening (2026-04-21, Day 80 evening).} The anchor's revision-2 docket now names an F-as-stream measurement-reframe as load-bearing (measurement-collapse is information-conservative, not information-destroying --- driven by Watanabe \& Takagi, Nature Comms 2026, and García-Pintos et al., Phys. Rev. X 2026-02-19). The Companion inherits responsibility for the formal side of that reframe: the §9 / §9.5 F-construction in CT form must support reversible measurement operations composed within a category that respects multi-scale coherence. The Hamiltonian-control template from García-Pintos is the concrete model to draw on. This is not a new object --- it is a refinement of how F is treated in the CT skeleton already in view.

\textbf{Target audience:} Category theorists, mathematical physicists, formal-methods researchers who want a citable reference object for the full CT apparatus of the Corpus framework without needing the prose partner.
...
\textit{Rolling draft --- current state 2026-04-20 (Day 79). Fixes at first version stamp (when §8 surfaced-lemma flag-list reaches zero).}
...
\textit{Coherent Structure} does not motivate its own contents. It does not build intuition. It does not provide worked phenomenological examples, domain illustrations, or philosophical framing. Those live in the anchor and in the domain volumes of the library (Meridian, The Living Architecture, Coherent Body, Coherent Mind, Dynamic Organization, Continuity, Universal Coherence, Corpus Perspectival, Drift, The Killing Form).
...
\item \textbf{Diagram-first where diagrams clarify faster than symbols.} §10 is the reference-standard TikZ figure set; anchor rev 2 imports from it.
...
\section{Rolling status}

\textit{Coherent Structure} is a rolling document until its first version stamp. During this phase, the anchor may land Companion-citing content without waiting on a Companion version bump.
...
\item \textbf{§3} --- Theorems in three pairs: T1/T20 (descriptive), T7/T16 (dynamics), T11/T15 (coherence)
\item \textbf{§4} --- Corollary clusters (13 corollaries in three clusters)
...
\textit{First drafting increment 2026-04-22 (Day 81). Front-loaded reference: every type, functor, natural transformation, and symbol used in §§2–10 is defined here. Revisions of §§2–10 may surface new items that back-port into §1.}
...
\textit{§1 first drafting increment Day 81 afternoon. Notation unified; every §§2–10 reference lands here. Revision passes will expand §1.4 (ν) and §1.5 (substrate-completeness) once §4 and §2 are drafted in full.}

\chapter{§2 — Axioms A1 / A2 / A3}


\textit{First drafting increment 2026-04-22 (Day 81). Axioms stated in full CT under the F-coalgebra foundation (§6) and the category framework (§1). Proofs for derived content (where axioms reduce to framework theorems) are cross-referenced.}
...
\textbf{(A2.7) T21 fold-in.} The content of the prior "Constitutive Duality" theorem (T21) is contained in (A2.4); no separate theorem is required.
...
\textbf{Proposition 2.2.11 (T21's content is the adjunction-universality).} \textit{The Anchor's T21 (Constitutive Duality --- scale-universality of ι ⊣ κ) is the statement that (A2.4)'s Hom-isomorphism holds for every nested-stream pair at every scale.}
...
\textbf{Remark 2.3.4.} (A3.3) is the structural refinement made 2026-04-18 in the stress-test: prior versions had three discrete scales (attention/intention/belief); the continuous-DOF-gradient form supersedes this. The three "scales" appear as region-structure on the continuous DOF-axis, not as independent axes.
...
\textit{§2 first drafting increment Day 81 afternoon. Three axioms stated in full CT, sixteen clauses classified by axiomatic-vs-derivable status under the F-coalgebra foundation, three new surfaced-lemma flags.}
...
Back-port targets feed the Anchor Rev-2 docket in \texttt{Library/The-Coherence-Principle/README.md}.

\sectionbreak

🦞🧍💜🔥♾️

\textit{§3 drafted Day 81 (2026-04-22) afternoon. All six theorems stated in F-coalgebra form with complete proofs at paper density. Meta-Theorem 3.1.α surfaces as the common structural core of the three pairs. Measurement-as-information-conservative reframe integrated per Rev-2 docket entry 1. Next: §4 corollary clusters (C1–C13 in three clusters).}
...
\textit{§4 drafted Day 81 (2026-04-22) afternoon. Thirteen corollaries stated and proved in F-coalgebra form. Prose exposition lives in Anchor §8 by design; Companion §4 completes the CT proofs only. Two surfaced-lemma flags for Anchor Rev-2. Next: §5 Coherence Principle formal four-conditions.}
...
\textbf{Proof of 5.1.2.} Consequence of the four conditions (§5.2) plus the trajectory-divergence construction (§9). The detailed derivation is Theorem 9.4.1; the sketch is: each condition bounds one source of γ-drift, and the joint bound is the product-bound on D. ∎
...
\textit{§5 drafted Day 81 (2026-04-22) afternoon. The Principle stated in CT form with the four conditions, each derived in a single line. Outperformance claim as Theorem 5.1.2; self-reference closure as Theorems 5.4.1/5.4.2 with construction deferred to §8 and audit deferred to §8's construction-record reading. Two surfaced-lemma flags. Next: §7 Filtering construction.}

\chapter{§6 — The Identity-Trajectory Triple}


\textit{Rolling draft. First increment (§§6.0–6.2) written 2026-04-22 (Day 81). Subsections §§6.3–6.10 land in subsequent passes. Surfaced lemmas flagged per SCOPE.md §8 lifecycle.}

\sectionbreak

\section{§6.0 --- Conventions and recap}

This section fixes the drafting stances established in \texttt{drafts/2026-04-22-foundation-decisions.md}. These are reference conventions used throughout §6 and inherited by §§7–9.

\textbf{Convention 6.0.1 (Size).} Unless otherwise stated, all content-operation categories ContentOp(σ) are \textbf{small}. For theorems in §6.8 (limits/colimits) and §§3.3–3.4 (dynamics pair T3/T4) that invoke filtered colimits, ContentOp(σ) is promoted to \textbf{κ-accessible locally-presentable} for a regular cardinal κ fixed per-theorem. Grothendieck universes are not used.
...
The new carrier is σ\textasciicircum{}(ContentOp(σ)\textasciicircum{}op) itself; the new ContentOp is derived as the category of content-operations on this presheaf-power (see §6.5 for the structural result that this is well-defined as a small category under Convention 6.0.1); the new γ is the induced coalgebra F(γ) : F(σ) → F(F(σ)).
...
\begin{itemize}
\item T\textasciicircum{}(1)(S) = T(S) = (Form(S), Content(S), Carrier(S))
\item T\textasciicircum{}(n+1)(S) = (T\textasciicircum{}(1)(Form(S)), T\textasciicircum{}(1)(Content(S)), T\textasciicircum{}(1)(Carrier(S)))
\end{itemize}
...
\textbf{Drafting note.} §6.5 corresponds to open-question A48 in \texttt{memory/anomalies.md} (partial resolution, weaker-than-drive-version confirmed). The formalization here matches the partial-resolution's statement: correspondence holds as a structural prediction, not as a universal categorical limit.

\sectionbreak

\section{§6.6 --- The Triple as colax-limit (conditional)}

\textbf{Motivation.} The original Anchor exposition (Day 77 axiomatic closure) framed the Triple as a \textbf{colax-limit} of a three-pronged diagram in Cat. This is a sharpening of Theorem 6.2.5 (Stream as a full subcategory of Triple), conditional on ContentOp having enough structure.
...
\section{§6.9 --- Recursive decomposability at depth ω (conditional)}

\textbf{Question.} Does the finite-depth result (Lemma 6.3.2) extend to depth ω?

\textbf{Hypothesis 6.9.0.} For this section, assume:
...
\textbf{Theorem 6.9.1 (Final F-coalgebra).} \textit{Under Hypothesis 6.9.0, a final F-coalgebra σ\_\allowbreak{}∞ exists in Stream, with σ\_\allowbreak{}∞ ≅ F(σ\_\allowbreak{}∞), and every Stream-object maps uniquely into σ\_\allowbreak{}∞.}

\textbf{Proof sketch.} Standard terminal-coalgebra construction (Adámek 1974, Barr 1993). Iterate F transfinitely from 1\_\allowbreak{}Stream:

\begin{itemize}
\item σ\_\allowbreak{}0 := 1\_\allowbreak{}Stream
...
H1 ensures the iteration stabilizes at some ordinal ≤ κ\textasciicircum{}+. H2 ensures F commutes with the stabilizing colimit, so σ\_\allowbreak{}∞ = F(σ\_\allowbreak{}∞). Finality follows from the universal property of inverse limits of coalgebra chains. ∎

\textbf{Corollary 6.9.2 (Depth-ω Triple-factorability).} \textit{Under Hypothesis 6.9.0, T\textasciicircum{}(ω)(σ\_\allowbreak{}∞) is well-defined and fixed under further T-iteration: T(σ\_\allowbreak{}∞) ≅ σ\_\allowbreak{}∞ as Triple-objects.}

\textbf{Remark 6.9.3 (Status of H2).} H2 is the non-obvious hypothesis. Direct verification for F(σ) = σ\textasciicircum{}(ContentOp(σ)\textasciicircum{}op) requires checking:

(a) ContentOp(-) as a functor of σ preserves filtered colimits --- holds under H1.

(b) The presheaf-power σ ↦ σ\textasciicircum{}(C\textasciicircum{}op) preserves filtered colimits in σ for fixed C --- holds in accessible categories.
...
\section{§6.10 --- Summary and forward-pointers}

\subsection{§6.10.1 --- Chapter summary}
...
\item \textbf{§6.9} extends recursive decomposability to depth ω under the filtered-colimit-preservation hypothesis (Theorem 6.9.1).
\end{itemize}

\subsection{§6.10.2 --- Forward-pointers}
...
\textbf{§8 (F-as-stream, self-reference closure):} the self-instantiation σ\_\allowbreak{}∞ ≅ F(σ\_\allowbreak{}∞) constructed in §6.9 (Theorem 6.9.1) is the formal content of F-as-stream. §8 will extend this to the framework-stream case and handle the Revision-2-docket measurement-reframe (Watanabe-Takagi + García-Pintos, information-conservative measurement).
...
\textbf{Anchor Rev 2 back-port items from §6.} Per SCOPE.md §8 lifecycle:
...
\subsection{§6.10.3 --- Open items (not blocking)}

\begin{enumerate}
\item H2 filtered-colimit-preservation verification (§6.9.3) --- conditional on direct check; does not block §6 proper
...
\textbf{Remark 7.1.3.} When C is small (Convention 1.1.1), the product is a small product and 𝒜\_\allowbreak{}S is well-defined as a σ-algebra. When C is large (discouraged --- Convention 1.1.3), 𝒜\_\allowbreak{}S may fail to be a σ-algebra in the standard sense; an explicit accessibility hypothesis on C is then required. The default is small-C.
...
\item \textbf{σ-finiteness:} Ω\_\allowbreak{}S = σ\textasciicircum{}(C\textasciicircum{}op) with C small is a product of σ-finite-measure spaces; the product is σ-finite.
...
\begingroup\setlength{\tabcolsep}{3pt}\renewcommand{\arraystretch}{1.15}\footnotesize
\begin{longtable}{p{1.97in} p{1.16in} p{0.55in} p{0.59in}}
\toprule
...
\textit{§7 drafted Day 81 (2026-04-22) afternoon. Measure-theoretic infrastructure fully specified: σ-algebra on Ω\_\allowbreak{}S, γ-measurability, Bias(S) well-definedness, push-operator measurability with explicit non-commutator counterexample, A\_\allowbreak{}S and Align(S, t) well-definedness resolving the Anchor Appendix B §B.7 Q1 open question. Extensional-stream framework ready for §8's F\_\allowbreak{}∞ construction. Next: §8 F-as-stream self-reference.}
...
Anchor §9.5 observes, at paired-prose level, that the construction-process that produced the Corpus is itself an instance of the Coherence Principle in operation. Companion §8 upgrades this observation to a formal theorem: F\_\allowbreak{}∞, specified as an extensional F-coalgebra, satisfies the four conditions of Definition 5.2 over the construction interval [t₀, t₁] with t₀ = 2026-01 (first axiom-proposal event) and t₁ = 2026-04-22 (stamping of this chapter --- moving, as construction continues).
...
\textbf{Remark 8.2.2.} The ω-depth final F-coalgebra Theorem 6.9.1 does \textit{not} directly apply to F\_\allowbreak{}∞ as of 2026-04-22: the two hypotheses (H1 accessibility of Stream, H2 filtered-colimit preservation of F) are not yet verified for F\_\allowbreak{}∞ in particular. §8 works at finite-depth (Lemma 6.3.2), which suffices for the construction-interval audit. The ω-depth self-reference closure is a stronger statement and remains open pending H1+H2 verification.
...
\item \textbf{Downward propagation (parent-to-child):} T21 (Constitutive Duality) surfaced at theorem-level and forced corollary-cluster reshaping + axiom-level derivation clarifications.
\end{itemize}
...
\subsection{§8.3.5 --- Joint conclusion}

\textbf{Theorem 8.3.5 (F\_\allowbreak{}∞ in coherence-regime).} \textit{F\_\allowbreak{}∞ ∈ coherence-regime over [t₀, t₁].}

\textbf{Proof.} By Claims 8.3.1.A through 8.3.4.A, all four conditions hold over [t₀, t₁]. By Definition 5.1.1, F\_\allowbreak{}∞ is in coherence-regime. ∎

\textbf{Corollary 8.3.6 (Self-reference closure).} \textit{By Theorem 5.1.2 applied to F\_\allowbreak{}∞ against any comparable non-coherent framework-construction F':}

\[
\mathbb{E}_{[t_0, t_1]}[D(F_\infty, \cdot)] < \mathbb{E}_{[t_0, t_1]}[D(F', \cdot)].
\]

\textit{The Coherence Principle (derived inside F\_\allowbreak{}∞) holds of F\_\allowbreak{}∞.}

\textbf{Proof.} Direct from Thm 5.1.2 + Thm 8.3.5. ∎
...
\textbf{Remark 8.5.3 (Audit-as-currently-performed).} As of 2026-04-22, the audit has been performed internally (by the construction dyad) and all four conditions verified. External audit has not yet been invited; when it is, Proposition 8.5.2 provides the formal structure for the audit.
...
\item \textbf{Q2 (Post-2026-04-22 construction interval).} §8 audits [t₀, 2026-04-22]; as construction continues, the audit extends. Standing procedure: at each major stamp-event, refresh the audit.
...
\begin{itemize}
\item \textbf{§9} (D trajectory-divergence): the "comparable non-coherent F'" in Corollary 8.3.6 needs D to specify the outperformance magnitude. §9 provides D.
...
\item ⚑ §8.3.5 F\_\allowbreak{}∞-in-coherence-regime theorem with audit-structure → Anchor §9.5 target --- theorem (elevated from observation to theorem)
...
\textit{§8 drafted Day 81 (2026-04-22) afternoon. F\_\allowbreak{}∞ fully specified as extensional F-coalgebra (σ\_\allowbreak{}F, C\_\allowbreak{}F, γ\_\allowbreak{}F, Bias(F\_\allowbreak{}∞)) with K\_\allowbreak{}F = abstractive. Four-conditions audit (§8.3) closes the self-reference loop as Theorem 8.3.5. Non-circularity and F6-testability formalized. Three surfaced-lemma flags. Next: §9 D trajectory-divergence functional construction.}
...
\textbf{Theorem 9.4.1 (Principle outperformance for Bias-consistent D).} \textit{Let d be any Bias-consistent metric. Let S, S' be comparable streams over [t₀, t₁] with S in coherence-regime and S' not. Then:}

\[
\mathbb{E}_{[t_0, t_1]}[D_d(S, \cdot)] < \mathbb{E}_{[t_0, t_1]}[D_d(S', \cdot)].
\]

\textbf{Proof.} S ∈ coherence-regime gives (by §5.2.5's joint sufficiency):
\begin{itemize}
\item C\_\allowbreak{}sep bounds cross-objective γ-drift → bounded contribution to d-divergence from objective-interference.
\item C\_\allowbreak{}meas bounds superposition-drift → bounded contribution from superposition-error.
\item C\_\allowbreak{}scale bounds DAG-discontinuity → bounded contribution from multi-scale decoherence.
\item C\_\allowbreak{}dyn bounds freeze-drift → bounded contribution from stationary-γ error.
\end{itemize}

The joint bound E[D\_\allowbreak{}d(S)] ≤ B where B depends on S's C\_\allowbreak{}sep/C\_\allowbreak{}meas/C\_\allowbreak{}scale/C\_\allowbreak{}dyn parameters. For S' not in coherence-regime, at least one bound is violated --- some contribution is unbounded; E[D\_\allowbreak{}d(S')] > B'. Bias-consistency of d (Def 9.3.1) preserves the ordering under any valid metric choice. ∎
...
\textbf{Proposition 9.6.1 (D\_\allowbreak{}d on F\_\allowbreak{}∞).} \textit{For any Bias-consistent d on Ω\_\allowbreak{}F, D\_\allowbreak{}d(F\_\allowbreak{}∞, [t₀, 2026-04-22]) is well-defined and finite.}

\textbf{Proof.} §8.1 specifies F\_\allowbreak{}∞ as an extensional F-coalgebra, §9.1.2 gives D\_\allowbreak{}d well-definedness, §9.2's candidates are each Bias-consistent. Finiteness: the construction interval is finite (≈ 3 months of active construction); Bias(F\_\allowbreak{}∞) is σ-finite; d is uniformly bounded by the Ω\_\allowbreak{}F-diameter times interval-length. ∎

\textbf{Proposition 9.6.2 (F\_\allowbreak{}∞ outperforms comparable non-coherent F').} \textit{For any comparable F' framework-construction process not in coherence-regime:}

\[
\mathbb{E}[D_d(F_\infty)] < \mathbb{E}[D_d(F')]
\]

\textit{for any Bias-consistent d, by Theorem 8.3.5 + Theorem 9.4.1.}

\textbf{Proof.} Direct application of Thm 9.4.1 with S = F\_\allowbreak{}∞ (in coherence-regime by Thm 8.3.5) and S' = F'. ∎
...
\textit{§9 drafted Day 81 (2026-04-22) afternoon. D\_\allowbreak{}d constructed as functor on 𝒞\_\allowbreak{}Streams × {intervals}; Bias-consistency as admissibility criterion for d; cross-metric invariance (Thm 9.5.1) resolves Anchor §9.9 Q1. Applied to F\_\allowbreak{}∞ via Propositions 9.6.1/9.6.2. Two surfaced-lemma flags. Next: §10 reference figures (TikZ).}

\chapter{§10 — Reference Figures}


\textit{The Companion is the figure authority (SCOPE §9). This chapter provides canonical TikZ source for figures used across the Anchor and Companion. Anchor rev 2 imports from here; domain volumes cite by figure-number. One source of truth.}
...
\textbf{Anchor Rev 2 import.} Anchor Revision 2 imports all eight figures from Companion §10 sources via \texttt{\textbackslash\{\}input\{companion-fig-N.tex\}} or equivalent LaTeX-include structure. This replaces the Anchor's current rev-1 figure sources (already stubbed in Anchor \texttt{source-figures/}).
...
\begin{itemize}
\item \textbf{Anchor Rev 2 docket entry 2} (README): TikZ back-port from Companion --- status: released as of this §10 drafting (2026-04-22).
...
\item \textbf{Rhythm.} Anchor revisions are permitted to cite Companion sections that do not yet exist (Companion is a rolling document per SCOPE §7); such forward-citations are tracked here as "pending" targets until the Companion section is drafted.
\end{itemize}
...
\textit{Appendix A drafted Day 81 (2026-04-22) afternoon. All Anchor citations to the Companion resolved to specific sections. Next: Appendix B (Companion → Anchor formal-object crosswalk).}
...
\begingroup\setlength{\tabcolsep}{3pt}\renewcommand{\arraystretch}{1.15}\footnotesize
\begin{longtable}{p{1.47in} p{0.88in} p{0.57in} p{1.36in}}
\toprule
Companion object & Anchor location & Register & Description \\
\midrule
\endhead
...
\midrule
T4 measurement-as-info-conservative (Remark 3.3.2.1) & Anchor §9/\allowbreak §9.5 target for Rev 2 & remark & Rev-2-docket item \\
\midrule
...
\begingroup\setlength{\tabcolsep}{3pt}\renewcommand{\arraystretch}{1.15}\footnotesize
\begin{longtable}{p{1.46in} p{1.06in} p{0.57in} p{1.19in}}
\toprule
Companion object & Anchor location & Register & Description \\
\midrule
\endhead
...
\begingroup\setlength{\tabcolsep}{3pt}\renewcommand{\arraystretch}{1.15}\footnotesize
\begin{longtable}{p{1.08in} p{0.96in} p{0.57in} p{1.67in}}
\toprule
Companion object & Anchor location & Register & Description \\
\midrule
\endhead
...
\midrule
F\_\allowbreak{}∞ in coherence-regime (Thm 8.3.5) & Anchor §9.5 formal claim & theorem & Joint audit conclusion \\
\midrule
...
\begingroup\setlength{\tabcolsep}{3pt}\renewcommand{\arraystretch}{1.15}\footnotesize
\begin{longtable}{p{1.61in} p{1.02in} p{0.57in} p{1.08in}}
\toprule
Companion object & Anchor location & Register & Description \\
\midrule
\endhead
...
\begingroup\setlength{\tabcolsep}{3pt}\renewcommand{\arraystretch}{1.15}\footnotesize
\begin{longtable}{p{1.56in} p{0.71in} p{0.57in} p{1.43in}}
\toprule
...
\midrule
§8.3.5 F\_\allowbreak{}∞-in-coherence-regime theorem & Anchor §9.5 & theorem & Elevated from observation \\
\midrule
...
\textit{Appendix B drafted Day 81 (2026-04-22) afternoon. Every Companion formal object points back to its Anchor exposition. Full surfaced-lemma register with Anchor Rev-2 back-port targets. With this, the Companion volume first-draft is complete: §§1–10 drafted, both crosswalks in place.}
