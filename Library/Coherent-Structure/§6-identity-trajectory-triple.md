...
**Remark 6.8.5.1 (Vacuity, not an origin).** The initiality is genuine — unlike the terminal case, kind-respect holds here without dropping clause (iv), so 0_Stream is initial in **Stream** itself, not merely in **Stream**^{−K}. What it is not is *substantive*. 0_Stream has an empty carrier: it has no states, no content-operations and no trajectory, so the unique morphism 0_Stream → S carries nothing into S. That is vacuity, not an origin, and it is the exact dual of Remark 6.8.1.1: 1_Stream is a floor reached by forgetting everything, 0_Stream is a floor from which nothing is brought. Anchor §1.0 Property 5 denies "a substantive initial 'empty stream' lifting into all others", and this proposition supplies no such thing: the empty stream lifts into all others precisely because it lifts nothing. Nothing here contradicts the Anchor.
