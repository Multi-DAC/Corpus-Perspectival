...
| §1.0 γ-naturality (Property 4) | Companion Definition 1.6.3 (iii), Definition 6.1.3 (iii); demarcation at §9.4.1 | definition + tolerance-parameter | The coalgebra-commute clause **is** Property 4's equation γ' ∘ f_σ = F(f_σ, f_C) ∘ γ. It is hom-set membership (Definition 1.2.1), not a theorem, so it discharges nothing: A2.4 makes every nested pair an ι, and Anchor §1.0.2's *no other morphisms* clause leaves the filter empty. Anchor **Q1**'s second disjunct (*identify the class of streams for which it holds*) is answered at §9.4.1 — the class is {S : δ_scale(S) ≤ ε_scale}. Q1's **first** disjunct (*prove it from A1–A3*) is unavailable by design: a proof would force δ_scale ≡ 0, making ¬C_scale unsatisfiable and falsification row F4 unfalsifiable. **Not** Companion §1.4.3, which is the naturality of ν — a different transformation, and one whose proof defers to a section that does not develop it |
