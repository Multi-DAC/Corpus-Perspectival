...
**Remark 7.1.3.** When C is small (Convention 1.1.1), the product is a small product and 𝒜_S is well-defined as a σ-algebra. When C is large (discouraged — Convention 1.1.3), 𝒜_S may fail to be a σ-algebra in the standard sense; an explicit accessibility hypothesis on C is then required. The default is small-C.
...
3. **σ-finiteness:** Ω_S = σ^(C^op) with C small is a product of σ-finite-measure spaces; the product is σ-finite.
