...
**Definition 7.1.1.** *For a stream S = (σ, C, γ) with σ equipped with a σ-algebra 𝒜_σ (or the discrete σ-algebra if σ is countable), the **canonical σ-algebra** 𝒜_S on Ω_S = σ^(C^op) is the product σ-algebra:*
...
**Remark 7.1.3 (Regime placement).** This construction of 𝒜_S assumes C is small. Within that scope, §6.9.0 further partitions into Regime A (finite C; 𝒜_S is trivially a σ-algebra) and Regime B (countable C; 𝒜_S is the standard countable-product σ-algebra on σ^(C^op) with Kolmogorov-extension structure). Under Regime C (large C, out-of-scope per Convention 1.1.2), 𝒜_S fails to be a σ-algebra in the standard sense; an explicit accessibility hypothesis on C would be required, and no such framework extension is pursued here. The default throughout §7 is Regime A ∪ Regime B (small-C).
...
**Definition 7.3.1 (Bias(S)).** *For a measurable adequate F-coalgebra S = (σ, C, γ), Ω_S := F(σ) = σ^(C^op) derived rather than posited (Remark 6.1.2), and a reference measure μ on (σ, 𝒜_σ), the **Bias measure** Bias(S) is the signed measure on (Ω_S, 𝒜_S) defined by:*
