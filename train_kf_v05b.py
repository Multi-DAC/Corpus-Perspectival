"""
KF-Coupled Control v0.5b — Same Objective on BOTH Parameters (Control for v0.5)

v0.5 showed 38,963x H-module CV amplification with decoupled training.
v0.5b is the CONTROL: apply the SAME KF regularization to BOTH H and L modules.

If v0.5b also shows high amplification → the effect is just "KF reg works on HRM"
If v0.5b shows DEGRADATION → the effect is specifically about separation of concerns

This controls for the architecture difference between v0.4 (Qwen) and v0.5 (HRM).

Setup: Identical to v0.5 EXCEPT:
  - KF regularization computes CV on BOTH modules (not just H)
  - NO L-module gradient zeroing — KF gradients flow everywhere
  - Same lambda, same schedule, same data

Usage:
  python train_kf_v05b.py [--kf_lambda 1.0] [--kf_every 50] [--epochs 2000]
"""
import sys
import os
import json
...
def compute_both_modules_cv_differentiable(raw_model, device):
    """
    Compute commutator variance of ALL attention heads (H + L modules combined).
    DIFFERENTIABLE — stays in autograd graph for backward through ALL params.

    v0.5b control: KF reg flows through BOTH modules, not just H.
    This tests whether separation of concerns matters, or if KF reg just "works" on HRM.
    """
    head_matrices = []

    # Collect from BOTH modules
    for module in [raw_model.inner.H_level, raw_model.inner.L_level]:
...
def main():
    parser = argparse.ArgumentParser(description="v0.5b KF-coupled control (both modules)")
    ...
    parser.add_argument("--save_dir", type=str, default="/home/clawd/HRM/checkpoints/kf_v05b",
    ...
    print("=" * 70)
    print("v0.5b: KF-COUPLED CONTROL (BOTH modules)")
    print(f"  KF lambda: {args.kf_lambda} (H + L modules)")
    print(f"  KF every: {args.kf_every} steps")
    print(f"  Epochs: {args.epochs}")
    print(f"  Save dir: {args.save_dir}")
    print("  Control for v0.5: does separation matter, or does KF reg just work?")
    print("  If v0.5b degrades like v0.4 → separation IS the mechanism")
    print("=" * 70)
    ...
    for chunk_idx in range(num_chunks):
        ...
        for set_name, batch, batch_size in chunk_loader:
            ...
            # ─── KF REGULARIZATION ON BOTH MODULES (v0.5b control) ───
            if global_step % args.kf_every == 0 and args.kf_lambda > 0:
                # Compute differentiable CV on BOTH H and L modules
                both_cv = compute_both_modules_cv_differentiable(raw_model, device)
                # We MAXIMIZE CV → minimize -CV
                kf_loss = -args.kf_lambda * both_cv

                # Backward through ALL params — no L-module zeroing
                # (This is the v0.5b control: same KF reg, but NOT decoupled)
                optimizer.zero_grad()
                kf_loss.backward()

                # NO L-module gradient zeroing — KF flows everywhere
                optimizer.step()

                kf_reg_applications += 1
                chunk_kf_loss += both_cv.item()

                if kf_reg_applications % 10 == 0:
                    print(f"  [Step {global_step}] KF reg #{kf_reg_applications}: "
                          f"BOTH_CV={both_cv.item():.6e}  kf_loss={kf_loss.item():.6e}", flush=True)
    ...
    # ─── FINAL SUMMARY ───
    print(f"\n{'='*70}")
    print(f"v0.5b COMPLETE — KF-COUPLED CONTROL (BOTH MODULES)")
    print(f"{'='*70}")
    ...
    # Save trajectory
    traj_path = os.path.join(args.save_dir, "kf_trajectory_v05b.json")
    with open(traj_path, "w") as f:
        json.dump(kf_trajectory, f, indent=2, default=float)
    print(f"\nTrajectory saved: {traj_path}")

    # Copy to Windows
    win_path = "/mnt/c/Users/mercu/clawd/projects/Corpus Perspectival/kf_trajectory_v05b.json"
    try:
        import shutil
