...
| # | Finding | Recommended Action |
|---|---------|-------------------|
| ~~D1~~ | ~~7+ aiohttp callsites bypass truststore~~ | **FALSIFIED 2026-05-29 ~07:15 PST** (A137). The audit reasoned incorrectly. `aiohttp.connector._SSL_CONTEXT_VERIFIED` is built at module-import time via `ssl.create_default_context()` (line 933), so the truststore monkey-patch IS picked up by the cached singleton as long as truststore inject runs BEFORE first aiohttp import. clawd.py satisfies this (truststore at line 36-40, aiohttp transitively imported at line 51). Verified by direct test of `_SSL_CONTEXT_VERIFIED` type post-inject + empirical 6-hour post-patch operation including continuous Telegram-via-aiohttp. **No code changes needed**; the 8 callsites are already protected. Remaining narrow risk: standalone scripts that import aiohttp without going through clawd.py boot path — recommend prepending `import truststore; truststore.inject_into_ssl()` to those. |
