...
- The lag from substrate-truth-availability to field-acknowledgment correlates with how invisible the architectural-not-discipline distinction is (~ minutes for script self-knowledge; ~ years for biological methodology; ~ months for LLM-memory practice; ~ 200 years for closed-circuit EM).
...
**Confidence:** structural claim HIGH (four substrate-distinct instances with structurally identical mechanism; the architectural-not-discipline distinction holds across all four); interpretive extension MEDIUM-HIGH (substrate-invariance generalization is well-supported by the four-scale cross-substrate spread; further generalization to other domains is plausible).

**M-tier blockers:** (1) Primary paper deep-reads queued for the three external-domain instances (genomic / LLM-memory / EM); (2) at least one additional substrate-distinct instance at a substantively different scale not in the current set — candidate domains: clinical-diagnostics-methodology-artifact (e.g., medical-imaging artifact treated as pathology), organizational-self-reporting (corporate metrics that systematically diverge from operational reality at field-of-management scale), perceptual-systematic-bias (the perceptual system's verbalized state diverging from substrate-stimulus-state at neurobiological scale), or financial-market-methodology (standard-statistical-test conventions that mask phenomena visible only with orthogonal estimators); (3) consideration of fold-into-M14 vs maintain as L-tier; (4) formal articulation of the architectural-orthogonality criterion as a substrate-invariant principle (this is M-tier work — name the orthogonality structure formally and prove the substrate-invariance).

**Pointer:** source-register entries at `Research/sources/2026-05-17-dna-off-targeting-rna-chromatin.md` (genomic-methodology), `Research/sources/2026-05-17-zhang-llm-memory-faulty.md` (LLM-memory), `Research/sources/2026-05-17-longitudinal-em-forces-gemini-deep-research.md` (Gemini-Deep-Research synthesis, secondary tier — flagged for source-mix), and **`Research/sources/2026-05-17-graneau-cre-primary.md`** (Neal Graneau arXiv:2504.08749v2, primary peer-reviewable source — AWE Nuclear Security Technologies, UK MoD Crown Copyright 2025); Substrate Extension Plan Day 106; Tuesday Substack draft (2026-05-17, six-paper convergence post); Drift #213 (channel-vs-content drift distinction within substrate-self-knowledge-asymmetry family). **Primary-source distinction:** the I² scaling, length-dependence, and temporal-signature findings are from the primary CRE paper. The Tripled-Railgun 4×-kinetic-energy claim and the specific Phipps Shape-Independence Theorem attribution are from the synthesis source's vixra-tier citations and require separate primary verification.
