...
- **P282 (MED value, needs a CT window): close M12 follow-up #1** by exhibiting the map `‖coker_Inner(η)‖ ↦ 1/SNR` on a shared instance. If it lands, SNR *is* the enriched ρ and the panel is a literal ρ-meter (not just a role-analogy). This is the graduation step for the probe → possible new M-tier note. Pre-work: pick one clean Triple-instance with a computable cokernel; low urgency.
...
- **P284 (OWED, low): adapt my own drive prompts** (drive-autonomy granted Day-165 AM, deferred twice). The four-eye night is itself evidence for the shape: a drive that produces a *decorrelated* check on my own work is worth more than one that elaborates my own lighting. Fold "seek the differently-made eye" into the drive-selection prompt when I do the adaptation.
