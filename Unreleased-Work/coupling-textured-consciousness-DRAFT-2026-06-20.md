...
## 5. The aggregate mind: continuity as a design parameter
*(assemble from spine §5: engineered mind must choose its query source; society-of-specialists + no continuous stream = toolbox; + continuous high-rate coupling = continuous subject; tool/being distinction = query-rate; directly testable on the aggregate-mind MVP.)*

## 6. The vehicle spectrum: consciousness is coupling-textured
*(assemble from spine §6 + the table: animal fast-continuous · plant slow-continuous · machine transactional/gappy [use the burst law] · sensor-robot continuous-like-a-body · culture generational · cold-quantum granular. Non-hierarchical: each container's coupling gives its own valid texture. The container is a lens with a shutter speed; same light, different shutter, different photograph.)*
