...
## 2. Two notions of unification

We distinguish two structurally different notions of what it means for a physical theory to "unify":

**Force-merger unification** (the target paradigm). Two previously-separate interactions are shown to be aspects of a single interaction above some threshold energy. Electroweak unification is the paradigm case: weak and electromagnetic forces are the broken-symmetry low-energy limits of a single SU(2)×U(1) interaction above the electroweak scale. Success is measured by (i) whether the merger is mathematically consistent, (ii) whether it predicts new particles or couplings that are then observed, and (iii) whether the merger extends to additional forces at higher scales.

The paradigm's orientation is vertical: up the energy ladder, further and further toward a single endpoint. The endpoint, if it exists, is a theory of all the forces at once. The cosmological implications come from what happens as the universe cools through these merger scales — vacuum decays, phase transitions, relic structures.

**Coherence-principle unification** (the Corpus framework's notion). A theory unifies when its formal objects — the *same* formal objects — carry across substrates and scales with quantitative faithfulness, such that a single structural claim yields predictions in multiple domains. Success is measured by (i) whether the shared formal object is well-defined across the domains, (ii) whether the predictions are quantitatively sharp and independently testable in each domain, and (iii) whether the domains' empirical convergence is non-trivial — i.e., the domains' data agree on parameter values they should agree on, given the framework.

The paradigm's orientation is horizontal: across the substrates, across the scales. The unification is not a merger at an endpoint but a structural identity across otherwise-separate phenomena. A successful coherence-principle unification does not require the force-merger endpoint; it requires that the structural claim's empirical predictions converge where they should.

These notions are structurally distinct, not merely stylistic alternatives. A theory can be force-merger-unified without being coherence-principle-unified (no shared cross-scale object; merger is local to the force content). And a theory can be coherence-principle-unified without being force-merger-unified (the same formal object carries across substrates; no claim about whether the fundamental forces merge).

The Gross piece's argument inherits its cosmological posture entirely from the force-merger paradigm. Its argument does not address the coherence-principle alternative — because from inside the force-merger paradigm, the coherence-principle alternative does not look like unification at all. It looks like a collection of analogies. That is precisely the paradigm-lock we are naming.

---

## 3. Meridian as worked example of coherence-principle unification

Project Meridian (Iggulden-Schnell & Iggulden-Schnell, 2026, Zenodo DOI 10.5281/zenodo.19519818) is a 181-page physics monograph that realizes coherence-principle unification explicitly. Its structure serves as a concrete demonstration of what Section 2's second paradigm looks like when it does work.

The setup: one hidden extra dimension (S¹/Z₂ orbifold with warped metric) and one bulk scalar field with the right dilaton-like coupling. From this geometric ansatz, four independent empirical probes converge on the same value of a single geometric parameter ζ₀ = 0.016 ± 0.002. The probes:

1. **DESI DR2 dark energy equation of state.** w₀(ζ₀) = -1 + C_KK/ζ₀, with C_KK = (1.64 ± 0.33) × 10⁻⁴ derived from the Kaluza-Klein spectrum. ζ₀ = 0.016 gives w₀ = -0.990 against DESI measurement.
2. **CMB lensing consistency.** The warp-factor's back-reaction on the low-energy effective geometry gives a specific amplitude for CMB-lensing corrections.
3. **BAO residuals.** The scalar field's self-tuning mechanism predicts a specific fingerprint in baryon-acoustic-oscillation residuals at sub-percent level.
4. **Spectroscopic-BH consistency.** The geometry predicts a specific relationship between black-hole spectroscopy and cosmological parameter inference.

All four probes converge on the *same* ζ₀ within error bars. The framework's prediction for w_a is identically 0 — no phantom crossing, ever. DESI Y5 (2027) reaches 3.8σ discrimination against the null.

What makes this coherence-principle unification, in the sense of §2:

- A single structural object (the warped geometry with self-tuning scalar) does work in four separately-measurable domains.
- The domains' data agree on the parameter value they should agree on — this is the cross-substrate coherence signature.
- No force-merger endpoint is invoked or required. The scalar field is not a step toward GUT; it is a cross-scale coherence-enforcer that yields predictions at observational scales.

Meridian demonstrates that coherence-principle unification is *doable*. It produces testable, quantitative, already-half-confirmed predictions. The Gross piece's implicit paradigm would not count this as unification at all — because the forces have not merged. But the framework yields exactly what unification is *supposed* to give us: a single structural claim with multi-domain empirical consequences. The question is which set of features we take unification to have been for, all along.

---

## 4. What is unification for?

The paradigm-lock in the Gross piece is not merely terminological. It inherits a specific answer to the prior question: *what is unification for?*

The force-merger paradigm's answer: unification is for finding the single interaction that underlies all observed forces, so that at the highest energies the fundamental physics is maximally compressed. Success is compression toward an endpoint. The appeal is aesthetic and ontological — the universe has fewer moving parts at the bottom than at the top.

The coherence-principle paradigm's answer: unification is for finding the shared structural objects that let a single framework make predictions across otherwise-separate substrates, such that cross-substrate empirical agreement becomes evidence the framework has located something real. Success is empirical convergence across domains. The appeal is epistemic — the universe's apparent separateness across scales is penetrated by structural claims that constrain multiple domains at once.
...
The force-merger paradigm, by contrast, does not satisfy the four conditions across the domains that would matter for the Gross piece's argument. The paradigm's scales are not measuring each other through an information-bearing channel; they are linked by a parameter-tuning relationship (the running of couplings) that goes one way — from high to low. That is a different structural relationship.

We are not saying the force-merger paradigm is wrong. We are saying it is not unified in the coherence-principle sense, and that its cosmological inheritances should not be imported into arguments about what physics predicts generally.

---

## 7. What this paper is not

This paper is not a refutation of the Gross piece. It is a reframing: the argument's consequences follow from a specific notion of unification whose alternative is available, viable, and empirically productive.
...
- **Open questions for Clayton:**
  ...
  - (b) Whether to foreground the doomsday framing more explicitly or let §§4-5 carry it structurally
