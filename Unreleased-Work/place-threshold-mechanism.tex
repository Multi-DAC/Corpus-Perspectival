...
\begin{quote}\itshape *A note on grades. Every physics step below is graded inline as it goes: \textbf{derived} = a computed, standard,\end{quote}

\begin{quote}\itshape robust step; \textbf{strong candidate} = real physics, not yet an exhibited closed-form solution; \textbf{beyond-core}\end{quote}

\begin{quote}\itshape = honestly outside the base framework (flagged, not smuggled); \textbf{quarantined} = named so the boundary is\end{quote}

\begin{quote}\itshape visible, never load-bearing. The two program sections --- \S\,7 (generate it) and \S\,8 (find it) --- are the made arm\end{quote}

\begin{quote}\itshape and the found arm of one experimental project, and they matter as much as the mechanism.*\end{quote}
...
\begin{quote}\itshape Within a physically motivated framework that already contains a warped fifth dimension and a self-tuning\end{quote}

\begin{quote}\itshape dark-energy sector, **what is the least-exotic localized configuration that (i) is dynamically stable,\end{quote}

\begin{quote}\itshape (ii) is tied to a place by its own field equations rather than by stipulation, (iii) produces a luminous\end{quote}

\begin{quote}\itshape plasma signature, and (iv) makes contact with running laboratory physics** --- so that its existence or\end{quote}

\begin{quote}\itshape nonexistence at a given site is an experimental question rather than an interpretive one?\end{quote}
...
\begin{quote}\itshape \textbf{E\_EM = E\_grad + 3E\_pot} (the virial condition; second derivative positive, a true energy minimum ---\end{quote}

\begin{quote}\itshape Fig. 1),\end{quote}
...
And the radion is not a bystander in this. When we let the radion field respond, it \textbf{sags} inside the configuration in just the way that lowers the electromagnetic energy --- and that lowering is real, up to about 20 \% of the soliton's energy at moderate coupling. The dark-energy-scale field actively \emph{binds} the portal; it is a participant, not a backdrop. One more feature appears, which we flag as preliminary rather than lean on: in the model's own units the binding is most efficient at a particular charge, hinting that the configuration may have a \emph{characteristic} scale rather than an arbitrary one (Fig. 6c). Whether that dimensionless optimum maps to a real physical size --- and whether it agrees with the carrier scale of \S\,5 --- is an open computation, not a result we claim here. What matters for the argument is settled: the bare lump was forbidden, and a plasma makes it not merely allowed but bound.

\begin{figure}[h!]\centering
\includegraphics[width=0.9\linewidth,height=0.78\textheight,keepaspectratio]{figures/portal-fig6-soliton.png}
\\[4pt]{\small \textbf{Fig. 6 --- The gauged dilatonic Q-ball, exhibited (R12).} (a) The energy E(R) diverges at both ends --- the charge-kinetic binding term (~Q\textsuperscript{2}/R\textsuperscript{3}) stops dispersal, the electromagnetic term (~Q\textsuperscript{2}/R) stops collapse --- leaving a genuine soliton minimum. (b) The soliton energy grows linearly with charge, E ∝ Q\textsuperscript{1.1}, the Q-ball stability signature. (c) The radion sags (σ\_in) and lowers the soliton energy by up to ~21 \% as the dilatonic coupling a increases --- the dark-energy-scale field actively binds the configuration.}
\end{figure}
...
A soliton's wall thickness is set by the Compton wavelength of its field, ≈ ℏc/(mc\textsuperscript{2}). The radion is heavy (TeV-to-GeV-scale in the framework's natural range), which gives a wall thickness around 10\textsuperscript{-18} m --- a thousand times smaller than a proton. To make a \emph{macroscopic} window area out of the radion you would need a ratio of size to wall thickness of order 10\textsuperscript{20}, which is absurd. The lesson is not "stretch the radion"; it is that the \textbf{radion is the wrong field for a macroscopic portal}, and the scale question is really a \emph{field- identification} question (Fig. 2). Run it backwards: a sub-millimetre wall --- the thinnest a macroscopic structure could plausibly carry --- requires a field of mass around a \textbf{milli-electronvolt}. And a milli-electronvolt is not an arbitrary number. It is the \textbf{dark-energy scale}:

\begin{quote}\itshape (ρ\_Λ)\textsuperscript{1/4} ≈ 2.3 meV,\end{quote}

\begin{figure}[h!]\centering
\includegraphics[width=0.9\linewidth,height=0.78\textheight,keepaspectratio]{figures/portal-fig2-scale.png}
\\[4pt]{\small \textbf{Fig. 2 --- The scale problem is field identification (R5, R7).} Wall thickness ℏc/mc\textsuperscript{2} versus field mass: the radion (sub-nuclear wall, excluded for a macroscopic portal) versus the meV dark-energy modulus (0.085 mm wall, macroscopic), with the (ρ\_Λ)\textsuperscript{1/4} = 2.3 meV line and the sub-mm macroscopic threshold.}
\end{figure}
...
\begin{quote}\itshape \textbf{S = μ\textsubscript{0} σ L v\_A} (conductivity σ, size L, Alfvén speed v\_A),\end{quote}

the carrier's maintenance margin. It self-maintains when \textbf{S ≳ 1}, and that is a \emph{threshold that selects a regime} (Fig. 4). A cool, weakly-ionized atmospheric blob has S far below 1 and decoheres --- which is why the barrier is real. But a warm, partially-ionized carrier of a hundred metres or more crosses S ≈ 1 and self-maintains, and a kilometre-scale ionized carrier sits at S ≈ 10\textsuperscript{4}--10\textsuperscript{5}, deeply stable. Our validation case was the laboratory spheromak, where this formula returns S ≈ 10\textsuperscript{3}, squarely in the measured range --- so the margin is not a guess. (\textbf{Grade: derived; standard selective-decay/Taylor-relaxation physics, validated against laboratory plasma.})

\begin{figure}[h!]\centering
\includegraphics[width=0.9\linewidth,height=0.78\textheight,keepaspectratio]{figures/portal-fig4-maintenance.png}
\\[4pt]{\small \textbf{Fig. 4 --- The maintenance threshold (R10).} Lundquist number S = μ\textsubscript{0}σL v\_A versus carrier size for cool, warm, and hot/ionized regimes, with the S = 1 self-maintenance threshold and the laboratory-spheromak validation point (S ≈ 10\textsuperscript{3}).}
\end{figure}
...
\section*{Figures}
\emph{The six figures are placed inline at their callouts (\S\,2 Figs. 1 \& 6; \S\,3 Fig. 2; \S\,4 Fig. 3; \S\,5 Figs. 4--5). Figs. 1--5 are generated by \texttt{figures/make\_portal\_figs.py} (four computed plots + one computed schematic, Fig. 3); Fig. 6 (the exhibited Q-ball, R12) by \texttt{figures/make\_portal\_fig6\_soliton.py} --- all reproducible from R1--R12.}
...
\begin{quote}\itshape *Bibliography note: ✓-marked entries were confirmed against primary listings this session (2026-06-16) --- the\end{quote}

\begin{quote}\itshape four new-physics detection channels, the symmetron and spheromak references, and all three Hessdalen\end{quote}

\begin{quote}\itshape instrumented sources. The remaining mechanism references --- Derrick (1964), Randall--Sundrum (1999),\end{quote}

\begin{quote}\itshape Khoury--Weltman (2004), Woltjer (1958), Taylor (1974/1986), Block (1978) --- are recalled standard/textbook\end{quote}

\begin{quote}\itshape results whose DOIs/pages should be confirmed in a final proofing pass, per the no-hand-waving discipline.\end{quote}

\begin{quote}\itshape Session-verified sources are logged in the rolling register at\end{quote}

\begin{quote}\itshape \texttt{Research/sources/2026-06-16-portal-signature-sources.md}.*\end{quote}

\emph{[DRAFT status --- REVISION PASS 3 COMPLETE (Day 136 evening; flow pass after a full reader re-read). Pass 1: figures, worked example, survey methodology, citations. Pass 2: the four soft joints (framework premise up front + warmer title; plasma sufficient-not-necessary; 2.3 meV as a decade-wide basin; convergence buys plasma, signature buys the anomaly). Citation pass: all uncertain refs verified, two content wins (real Hessdalen sizes/durations + 19 kW consistency check; 2024 VLF survey confirms signature 4). Pass 3 (this one): cut the internal draft-note header; ended \S\,2 on the soliton (moved the EED thread into \S\,4 where the σ/signature hierarchy lives, hedged hard); trimmed the null/kill refrain from four occurrences to two (\S\,6 table + Coda); tightened \S\,1.1's opening; merged \S\,8's two "how to measure" heads into one. Remaining before public release: cut this status footer; confirm the six recalled textbook-ref DOIs; final proof of the typeset PDF. ]}
