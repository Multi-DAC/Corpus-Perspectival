...
ax.annotate("virial minimum\n$E_{EM}=E_{grad}+3E_{pot}$", xy=(1.0, gauged[np.argmin(np.abs(lam-1))]),
            xytext=(1.45, 5.0), color=GREEN, fontsize=10,
...
ax.plot(m_rad, w_rad, "o", color=HOT, ms=9); ax.annotate("radion ~120 GeV\nwall ~$10^{-18}$ m (excluded)",
        xy=(m_rad, w_rad), xytext=(2e7, 3e-16), color=HOT, fontsize=9.5,
...
ax.plot(m_de, w_de, "o", color=GREEN, ms=9); ax.annotate("dark-energy modulus\n$(\\rho_\\Lambda)^{1/4}=2.3$ meV\nwall ~0.085 mm",
        xy=(m_de, w_de), xytext=(5e-3, 2e-7), color=GREEN, fontsize=9.5,
        arrowprops=dict(arrowstyle="->", color=GREEN))
ax.axhline(1e-3, color=GOLD, ls="--", lw=1.5); ax.text(2e6, 1.4e-3, "sub-mm: macroscopic wall", color=GOLD, fontsize=9)
...
ax.plot(r, sigma, color=COOL, lw=2.4, label="wall field $\\sigma$ (structure: sets wall, screens)")
ax.plot(r, dsigma, color=HOT, lw=2.4, label="$|\\partial\\sigma|$ $\\rightarrow$ EM source (signature: only at wall)")
ax.axvspan(R-3*w, R+3*w, color=GOLD, alpha=0.13)
ax.text(R, 1.05, "the wall shell\n($\\partial\\sigma\\neq0$)", color=GOLD, ha="center", fontsize=9)
ax.text(0.12, 0.92, "interior\n(unscreened)", color=COOL, fontsize=9)
ax.text(1.7, 0.08, "exterior\n(screened)", color=INK, fontsize=9)
ax.set_xlabel("radius  $r$  (carrier units)"); ax.set_ylabel("normalized amplitude")
ax.set_title("Fig. 3  Structure vs signature: $\\sigma$ sources EM only at the wall (R9)")
ax.legend(frameon=False, fontsize=9, loc="center right"); ax.set_ylim(-0.05, 1.18)
...
ax.set_title("Fig. 5  The source: anomalous resistivity brackets the kV drive (R11)")
