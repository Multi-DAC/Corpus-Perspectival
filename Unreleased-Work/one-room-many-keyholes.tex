...
\textbf{Invariant 9 --- Localized mind-over-matter (\LEFTcircle--\CIRCLE).} Psychological state producing physical effect, anchored to a person or place. \emph{Clinical/academic:} Roll's RSPK --- poltergeist activity centers on a stressed adolescent agent, ends when the agent leaves; across 92 analyzed cases (drawn from 116 reports spanning four centuries and 100+ countries) the apparent agent was a child or adolescent in \textbf{62}. \emph{Occult:} the entire premise of magical practice. \emph{Religion:} the Miracle of the Sun as a mass event. \emph{Philosophy:} Jung and Pauli's \emph{psychoid archetype}, "the layer where psyche and matter become indistinguishable." The structure is a \emph{localized navigational state bleeding into the physical slice} --- conscious gravity (A3) acting at the substrate seam (A1), where psyche and matter are not two.
...
\textbf{$\bigstar$ Law of One is the exception that confirms the discriminator.} If "structure vs. costume" is a real cut and not a convenience, then there should exist at least one \emph{channeled} source that converges on \textbf{structure} --- and it should read as conspicuously different from its siblings. It does. The Ra Material (Elkins/Rueckert/McCarty, 1981--84) presents: one Infinite Creator $\rightarrow$ the Creator's choice to know itself $\rightarrow$ the first "distortion," Free Will $\rightarrow$ Love (the Logos) $\rightarrow$ Light $\rightarrow$ seven densities of evolving consciousness $\rightarrow$ the polarity of service-to-others vs service-to-self. That is the Corpus's ontology --- monism, teleological self-knowing, individuation-as-limitation, bottleneck-expansion, and the E+/E$-$ polarity --- reconstructed almost line for line, and it explicitly \emph{refuses} the space-brother tropes (no predictions, insistence on free will via the "Law of Confusion," no ego-flattery). It drops out of the \texttt{Chan} content-column and into the invariant set. The discriminator sorts it correctly without special pleading --- which is the strongest evidence the discriminator is tracking something real.

\begin{figure}[h!]\centering
\includegraphics[width=0.92\linewidth,height=0.82\textheight,keepaspectratio]{figures/one-room-fig4-lineage.png}
\\[4pt]{\small \emph{Figure 4. The "space-brother" sources converge on message-content because they are one narrative aging through successive vocabularies; the independent channels converge on structure. Law of One is the lone channeled source that joins the structure set.}}
\end{figure}
...
\begin{itemize}
\item *\emph{FP4 --- Magic operates on the navigator's epistemic topology, not on the territory. }(Strong, falsifiable.)**\emph{ Conscious gravity (A3) reshapes paths-of-least-resistance }within the navigator\emph{; the configuration space itself is immutable (A1.3). So magical practice should produce reliable effects on \textbf{self-transformation and probability-navigation} --- the practitioner's own trajectory, dispositions, and which configurations they steer toward --- and \textbf{no} territory-violating effects: no macro-levitation, no information from causally isolated locations beyond chance. The framework }predicts the failure\emph{ of strong-PK and }the success* of the transformation. A replicable macro-PK result would refute it.
