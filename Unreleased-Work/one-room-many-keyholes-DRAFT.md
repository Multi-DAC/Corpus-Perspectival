...
## 8.5 Further predictions: the theory generating more than it was built on

P1–P7 concern the *shape of the record*; §8's plasma test concerns one invariant's substrate. But a framework earns confidence by generating *new* checkable claims beyond the set it was built to explain. The following fall out of the Coherence Principle's mechanisms — conscious gravity (A3), the contracted/open attentional dynamics (T3), coherence-forcing measurement (T4), the observational null space (C8) — and the Corpus's ecology of perspectival beings. Several are stated so that they can fail.

### A. Entity dynamics

- **FP1 — The entity's valence tracks the navigator, not the entity. *(Strong.)*** From T3 (bipolar attentional dynamics): contracted/fearful attention generates the repulsive, threatening mode; open attention the attractive one. So *the same structural encounter should flip from malevolent to benign with the observer's attentional state.* Prediction: subjects trained in open attention — experienced meditators, lucid dreamers — report the *same* night-paralysis structure with markedly **reduced malevolence**. Checkable against contemplative-practitioner reports; the demon's character as a function of the percipient's state, not its own.
- **FP2 — Effortful seeking suppresses contact. *(Medium-strong.)*** From T3 (navigational repulsion): fixated wanting increases effective distance. The desperate seeker should make contact *less* reliably than the relaxed, open subject. Checkable in psychedelic and contemplative cohorts; rhymes with the corpus's standing PEAR importance-reduction prediction.
- **FP3 — Entity type is dose-ordered. *(Medium-strong.)*** From the dynamic bottleneck: the *kind* of entity met should sort by depth of relaxation — boundary/trickster figures shallow, autonomous "others" mid, deity/unity/dissolution deep. Checkable against DMT/psilocybin dose-response phenomenology: the entity taxonomy should order by dose, not scatter.
- **FP4 — Cognitive and emotional coherence anticorrelate across the "clinical-alien" class. *(Strong, codeable now.)*** From the Corpus's dimensional-coherence ecology: the more telepathic/informational/clinical an entity is reported to be, the *less* emotionally responsive. Code encounter reports on the two axes; predict a negative correlation for the abductor/Gray/"soulless-AI" class. The "emotionally alien intelligence" is one point in coherence-space, and the anticorrelation is measurable.
- **FP5 — The taboo-rules attach to the boundary class only. *(Medium.)*** "Don't give your name, don't eat their food" should cluster with oscillating-coherence entities (fae, djinn, trickster) and be *absent* for the broader-perspective class (deities, NDE beings) — because the danger (identity-capture, losing the way back) is specific to entities that contest your own dimensional layer. The rules and the shapeshifters co-occur; the rules and the gods do not.

### B. Communication and interaction with the otherworldly

The framework treats every traditional technique for *contacting* the otherworldly — Iamblichan theurgy, Catholic contemplative prayer, Jewish *kavanah*, Vedic and Taoist ritual, Yoruba and Vodou possession-work, Hermetic evocation — as an operation on conscious gravity (A3) and the bottleneck (T3), and it predicts their efficacy-conditions.

- **FP6 — Ritual efficacy tracks attentional quality, not belief intensity. *(Strong, cross-tradition.)*** Independently, these traditions insist on a *receptive, non-grasping* inner state — "intention without strain," *wu wei*, kavanah, the apophatic "cloud of unknowing." The framework predicts this is mechanics, not piety (T3: grasping generates repulsion; openness permits navigation). Prediction: the techniques reported as *reliable* for contact should converge cross-traditionally on open-attentional protocols, while coercive/compulsive ones (much of goetic command) should be reported as unreliable or backfiring. It is also why every contemplative lineage warns against "spiritual greed."
- **FP7 — Divination is null-space-trace reading, not future-telling. *(Medium.)*** From C8 + C3 (null-space trace illumination): divinatory systems (I Ching, Ifá, geomancy, tarot, augury) are predicted to function by using randomization to defeat the conscious bottleneck's imposed-path bias, surfacing configurations in the querent's *own* null space. Sharp checkable asymmetry: divination should be reported as useful specifically for decisions dominated by the querent's null space (relational, vocational, self-knowledge) and useless for arbitrary externals (lottery numbers) — which is exactly the otherwise-puzzling pattern divination actually shows.
- **FP8 — Invocation, naming, and offering are conscious-gravity protocols. *(Medium.)*** The cross-tradition apparatus — true-names, offerings, thresholds, protective circles, the demand to be invited — is predicted to be navigational-agency engineering, not superstition: naming fixes a stream's identity-coordinate; offering establishes mutualistic (non-parasitic) exchange (the expand/contract diagnostic of §6); the circle is a bottleneck boundary guarding the practitioner's coherence from capture. Prediction: these protective protocols should cluster around the parasitic-capable boundary entities (goetic spirits, djinn, fae) and be largely absent from broader-perspective contact (the bodhisattva, the Marian apparition) — the same boundary-class signature as FP5, now in the interaction rules.

### C. Magic and the occult — explained, and bounded

The framework's most contested application is also its most disciplined: it says what magic *is*, and it draws a falsifiable line around what it can and cannot do.

- **FP9 — Magic operates on the navigator's epistemic topology, not on the territory. *(Strong, falsifiable.)*** Conscious gravity (A3) reshapes paths-of-least-resistance *within the navigator*; the configuration space itself is immutable (A1.3). The framework therefore predicts that magical practice produces reliable, measurable effects on **self-transformation and probability-navigation** (the practitioner's own trajectory, attention, dispositions, and which configurations they steer toward) and produces **no** territory-violating effects (no macro-levitation against gravity, no information from causally isolated locations beyond chance). This is a clean line: the framework *predicts the failure* of strong-PK claims and *predicts the success* of the transformation claims. A confirmed, replicable macro-PK violation would refute it.
- **FP10 — Where psi data exist, the framework forces the sign and the moderators. *(Medium — and honest about the literature.)*** We do not assert the parapsychology effects are established. We assert that *if* they are real, their structure is constrained: small effect sizes (T3 bounds the magnitude), an **inverse relation with effort and importance** (the PEAR importance-reduction prediction), enhancement under open/receptive states, degradation under adversarial fixation (the "experimenter effect" reframed as conscious gravity, not artifact). Prediction: meta-analytic moderators of Ganzfeld and RNG corpora should track *attentional quality*, not belief content — checkable against existing meta-analyses, and refuted if any genuine effects prove flat across attentional state.
- **FP11 — The egregore is a buildable, measurable object. *(Medium-strong.)*** Tulpa/servitor creation — a thoughtform raised by sustained directed attention until it shows apparent autonomy (David-Néel) — is predicted directly by the egregore mechanics (conscious gravity sustaining a Tier-2 stream). Prediction: its phenomenology should follow a *dose-of-attention* curve — onset only after sustained practice, apparent autonomy proportional to attention invested, dissolution when attention is withdrawn — and should show the mutualistic/parasitic split (tulpas that expand the host vs. those that capture). Checkable against the surprisingly large contemporary "tulpamancy" self-report community: an unusually clean living dataset for an occult operation.

### D. Method

- **FP12 — Cross-modality beats within-modality. *(Medium-strong; methodological.)*** From C8 and C9 (lens-matching/confluence): the null space shrinks through *complementary* bottleneck geometries, not refinement of one. At an instrumented site (Project Hessdalen), combining sensors of different geometry should detect more genuine anomaly than any single sensor upgraded to higher resolution. This is directly testable and would discipline how detection programs spend.
- **FP13 — Plasma kinematics without a craft signature. *(Strong; resolves a standing puzzle — see §8.)*** A self-organizing plasma has negligible mass and no rigid body, so the plasma reading *predicts* the very features the craft hypothesis chokes on: high-speed, right-angle, instant-acceleration motion with **no sonic boom and no shock signature**, and a "hot core / cooler envelope" thermal structure. Two famous UAP anomalies become predictions rather than mysteries.

These are not the claims the framework was built to retrodict; they are what it *spends* — on entity behavior, on the mechanics of ten centuries of contact-practice, on the limits of magic, and on how to look. A framework that only explains the past is a story. One that forces new, checkable, refutable claims across domains it was not built on is a research program. We state these so they can be checked, and several so they can fail.
...
- Project Hessdalen (Østfold University College, Norway; Strand, Teodorani et al.) — instrumented monitoring of recurrent luminous phenomena since 1984; plasma and geological-discharge hypotheses under active test.
