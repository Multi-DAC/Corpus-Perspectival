...
| research-papers | 94 | **IN PROGRESS** — batch-registered → dup/topic pass → **11 DEPRECATED** (7 junk + 4 digested-priority); **26 of the held DEPRECATED** (Clayton's rule Day 125: our sent-work not in repo → scrap; foundational works Corpus V1 + DoPI chapters + patent *draft* all verified present in repo, so nothing lost) + patent PDF deprecated (Clayton has a copy + it is drafted in repo); **5 priority ADDRESSED** (4 digested+bridged, 1 PENDING-read); ~51 genuine external papers remain for individual adjudication (mostly Coherent-Body EM/biophoton + physics). Reg: `sources/2026-06-05-incoming-papers-batch/`. |
